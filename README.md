...
When P90 demand for a product exceeds current stock + lead time, the inventory Lambda triggers an SNS alert to the purchasing team. Stockouts on trending products drop.
...
**Incremental cost over Phase 12: ~$2.20/day** when Forecast training is complete.

> Amazon Forecast: train the predictor once, export predictions, then delete the predictor. You keep the exported CSVs in S3. Retraining is a deliberate action — not a continuous cost.
